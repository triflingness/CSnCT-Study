...
- 하드웨어의 기능을 실행하기 위해 필수적인 작업(명령어 해석, 하드디스크에 데이터 저장, 주변장치와의 통신)을 하거나 응용 소프트웨어의 실행을 지원하는 소프트웨어
  - 운영체제: 컴퓨터 자원을 관리하고 응용 프로그램의 실해 환경을 제공하는 소프트웨어
  - 장치 드라이버: 컴퓨터에 온라인을 연결된 주변 기기를 제어하느 운영체제 모듈
  - 컴파일러와 인터프리터: 고급언얼 작성된 원시 프로그램(source program)을 컴퓨터가 이해하는 저급 언어로 변환하는 소프트웨어로서 일종의 번역기
  - 링커: 다수로 분할하여 작성되 프로그램ㅇ 의 생성된 목적 프로그램(object program) 또는 라이브러리 루틴을 결합하 실행 가능한 하나의 프로그램으로 연결하는 소프트웨어
  - 로더: 저장장치에 보관된 프로그램을 읽어 메인 메모링 적재하 후 실행 가능한 상태로 만드는 소프트웨어
